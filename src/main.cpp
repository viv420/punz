...
const string strMessageMagic = "PUNZ Signed Message:\n";
...
{  
	...
	//Day 4
	if(pindexBest->nHeight > 720)
    {
        nSubsidy = 1 * COIN; 
		return nSubsidy + nFees; 
    }
	//Day 5
	if(pindexBest->nHeight > 960)
    {
        nSubsidy = 10 * COIN; 
		return nSubsidy + nFees; 
    }
	//Day 6
	if(pindexBest->nHeight > 1200)
    {
	...
	//Day 8
	if(pindexBest->nHeight > 1680)
    {
	    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10;
		return nSubsidy + nFees; 
	}	
	//Day 9
	if(pindexBest->nHeight > 1920)
    {
        nSubsidy = 1000 * COIN; 
		return nSubsidy + nFees; 
    }
	//Day 10
	if(pindexBest->nHeight > 2160)
    {
	    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 100;
		return nSubsidy + nFees; 
	}	
	//Day 11
    if(pindexBest->nHeight > 2400)
    {
        nSubsidy = 1 * COIN; 
		return nSubsidy + nFees; 
    }	
...
{  
	...
	else if(pindexBest->nHeight < 2300)
    {
        nSubsidy = 300 * COIN; 
		return nSubsidy + nFees; 
    }
	...
	else if(pindexBest->nHeight < 2700)
    {
        nSubsidy = 300 * COIN; 
		return nSubsidy + nFees; 
    }
	...
	else if(pindexBest->nHeight < 2900)
    {
        nSubsidy = 100 * COIN; 
		return nSubsidy + nFees; 
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PUNZ", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "OMG OMG OMG";
        CTransaction txNew;
        txNew.nTime = 1464078152;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1464078152;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 941380 : 941380;
        ...
        assert(block.hashMerkleRoot == uint256("0x97f6b5c638789e887a5acf025baaa011d9487857a0c3c4892d57eefe3ba8f644"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x4a, 0xc5, 0x71, 0xc2 };
